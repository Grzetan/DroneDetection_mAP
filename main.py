...
def load_predicions(path: str):
    ...
    for i, row in enumerate(data[1:]):
        # Parse to int and transform from x,y,w,h to x1,y1,x2,y2
        tmprow = [int(a) for a in row[:-3]]
        tmprow[3] = tmprow[1] + tmprow[3]
        tmprow[4] = tmprow[2] + tmprow[4]
        tmprow.append(float(row[-1]))
        data[i] = tmprow
...
def mean_average_precision(
    ...
    TP_cumsum = np.cumsum(TP, dim=0)
    FP_cumsum = np.cumsum(FP, dim=0)
    recalls = TP_cumsum / (total_true_bboxes + epsilon)
    precisions = TP_cumsum / (TP_cumsum + FP_cumsum + epsilon)
    precisions = np.cat((np.tensor([1]), precisions))
    recalls = np.cat((np.tensor([0]), recalls))
...
def main():
    preds = load_predicions("./predictions/coordinates_1.csv")
    # for row in preds:
    #     print(row)
        
    labels = load_labels("./labels/connected_1.csv")
    for row in labels:
        print(row[0])
    print("XD: ", len(labels))
